...
#
# Outcomes: same scope as build_trends.R (in_cleaned_csv, non-platform-
# indexed, renderable response_type).
#
# Grouping vars:
...
tryCatch({
  ...
  cat(sprintf("Outcomes in scope: %d\n", length(outcomes)))
  ...
  for (v in outcomes) {
    ...
    for (g_col in grouping_vars) {
      ...
      for (w in waves) {
        ...
        # Groups present in this wave for this grouping var
