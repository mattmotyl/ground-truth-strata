...
# === WELLBEING — ls002a-l (all 12 in W1-W4; a/b/e/f dropped from battery in W5-W6) ===

LS002_ITEMS = [
    # (vn, question_text, clean_name, dropped_in_w5_w6, is_reverse_coded)
...
for vn, qtext, clean, dropped_in_w5_w6, reverse in LS002_ITEMS:
    waves_p = [1, 2, 3, 4] if dropped_in_w5_w6 else ALL_WAVES
    change_notes = None
    if dropped_in_w5_w6:
        change_notes = (
            "Battery shortened in W5-W6: the column persists in uas518.csv and uas519.csv but "
            "every value is the UAS '.a' (skipped) sentinel — n_non_sentinel = 0. The original "
            "PDF empty-elseif branches correctly reflected this; Phase 1 followup #5's "
            "'FALSE ALARM' reversal was based on column-presence verification that conflated "
            "structure with data. Re-verified 2026-05-25 by direct raw-CSV value inspection: "
            "ls002a/b/e/f are W1-W4 only. The other 8 ls002 items (c/d/g/h/i/j/k/l) carry "
            "through all 6 waves with ~2400-2900 non-sentinel values per wave."
        )
...
dictionary = {
    "_meta": {
        ...
        "note": (
            ...
            "Re-verified 2026-05-25 against raw CSV values (not just column presence) "
            "during a Phase 2 cleaning probe: ls002a/b/e/f and us019_hours/_minutes had "
            "their waves_present corrected — see phase1_followups entries 5 and 11."
        ),
        ...
        "phase1_followups": [
            ...
            },
            {
                ...
                "status": "RE-VERIFIED 2026-05-25 — original PDF reading was correct",
                "summary": (
                    "Re-verified during a Phase 2 cleaning probe by inspecting raw CSV VALUES "
                    "(not just column presence): ls002a, ls002b, ls002e, ls002f columns DO "
                    "exist in uas518.csv (W5) and uas519.csv (W6), BUT every value is the UAS "
                    "'.a' (skipped) sentinel — n_non_sentinel = 0 in both waves. The other 8 "
                    "ls002 items (c/d/g/h/i/j/k/l) carry through all 6 waves with ~2400-2900 "
                    "non-sentinel values per wave, confirming this is a deliberate battery "
                    "shortening (drop the 4 items, keep the other 8), not a coding bug or "
                    "missing data event. waves_present for ls002a/b/e/f corrected to [1,2,3,4]. "
                    "The 2026-05-24 'FALSE ALARM' reversal was an incomplete verification "
                    "(checked column presence but not values) — the original PDF empty-elseif "
                    "observation was correct. The cleaning layer was always behaving correctly; "
                    "only the dictionary's waves_present was wrong."
                ),
            },
            {
                "priority": "HIGH",
            ...
            {
                "priority": "MEDIUM",
                "id": "us019-hours-minutes-w6-absence",
                "status": "DISCOVERED 2026-05-25 (Phase 2 cleaning probe) — REAL DATA LIMITATION",
                "summary": (
                    "us019_(hours|minutes)_<N>_ columns do not exist at all in the W6 raw "
                    "CSV (uas519.csv). Present in W4 (40 columns) and W5 (38 columns) only. "
                    "Originally documented as W4-W6; corrected to [4,5]. transform_data.R's "
                    "`matches('^us019_(hours|minutes)_\\\\d+_$')` selector silently selects "
                    "zero columns in W6, which is correct behavior — no code change required."
                ),
            },
        ],
    },
